...
def save_partial_frames_ply(frame, surface_pts_normals):
...
if __name__=="__main__": # $python visualization_bunny.py bunny/data
    ...
    path = "/home/xiaoyan/3D/4DRecon/4DRep_DeepSDF3D/data/DeformingThings4D/animals/bear3EP_Agression"

    filenames = sorted(os.listdir(os.path.join(path, 'global_w_normal_obj')))[:-1]
    num_frame = len(filenames)
    assert num_frame == 100
    viewpoints_dir = os.path.join(path, 'frame')
    ...
    partial_obj_dir = os.path.join(path, 'partial_obj')
    if not os.path.exists(partial_obj_dir):
        os.makedirs(partial_obj_dir)
    ...
    for frame in range(num_frame):
        ...
        for viewpoint in frame_viewpoint:
            ...
            mesh = trimesh.load(os.path.join(viewpoints_dir, viewpoint, f'partial_objs/{frame:04d}.obj'))
            frame_points.extend(mesh.vertices)
            frame_normals.extend(mesh.vertex_normals)
            frame_faces.extend((mesh.faces+offset).tolist())
        ...
        for face in frame_faces:
            face_pts.append([frame_points[face[0]].tolist(), frame_points[face[1]].tolist(), frame_points[face[2]].tolist()])
        # import ipdb; ipdb.set_trace()
        for idx in range(len(frame_faces)):
            face_idx = frame_faces[idx]
            face_pt = face_pts[idx]

            if face_pt not in unique_faces_pts:
                unique_faces_idx.append(face_idx)
                unique_faces_pts.append(face_pt)
    

        # import ipdb; ipdb.set_trace()
        unique_indices = np.unique(unique_faces_idx)
        unique_vertices = np.array(frame_points)[unique_indices]
        unique_normals = np.array(frame_normals)[unique_indices]
        assert len(unique_vertices) == len(unique_normals)

        face_map = {old: new for new, old in enumerate(unique_indices)}
        faces = np.vectorize(face_map.get)(unique_faces_idx)

        unique_faces = np.unique(faces, axis=0)

        new_mesh = trimesh.Trimesh(vertices=unique_vertices, faces=unique_faces, vertex_normals=unique_normals)
        new_mesh.export(os.path.join(partial_obj_dir, f'{frame:04d}.obj'))
