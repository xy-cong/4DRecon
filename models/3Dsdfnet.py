...
class SdfNet(nn.Module):
    ...
    def get_PD_loss(self, time, state_info):
        ...
        sdfs = implicit_utils.sdf_decode_from_meshgrid(self.config, self, time, resolution=self.meshgrid_size, max_batch=int(2 ** 14), x_range=x_range, y_range=y_range, z_range=z_range)
        # sdf.shape = (B, meshgrid_size, meshgrid_size, meshgrid_size)
        # must be positive inside and negative outside
        ...
        state_info['sdf_topo_loss'] = (sdf_topo_loss / Group_num).item()
        state_info['sdf_topo_weight'] = self.config.loss.Topology_loss_coefficient
        sdf_topo_loss = (sdf_topo_loss / Group_num) * self.config.loss.Topology_loss_coefficient
    ...
    def extract_iso_surface(self, batch_vecs, cfg, batch_dict=None):
        ...
        for b in range(B):
            ...
            if cfg.get('simplify_mesh', True):
                ...
                with open('work_dir/0808_DeformingThings4D_partial_encoder_xyz_no_t_sample/run/results/output.obj', 'w') as file_obj:
                    file_obj.write(mesh_sim.export(file_type='obj'))
