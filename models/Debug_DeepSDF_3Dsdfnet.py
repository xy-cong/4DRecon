...
from models.deepsdfnet_base import SdfDecoder_Modify
...
class SdfNet(nn.Module):
    def __init__(self,
        ...
        if config.model.SDF_type == 'ModifySDF':
    ...
    def forward(self, batch_dict):
        ...
        idx = batch_dict['idx'].reshape(-1, 1) # (B, 1)
        lat_vec = batch_dict['lat_vec'] # (B, latent_dim)
        query = batch_dict['point_samples'].clone().detach().requires_grad_(True) # (B, S, query_dim)

        B, S = query.shape[0], query.shape[1]

        sdf_pred = self.decoder(repeat(lat_vec, 'B d -> (B S) d', S=S),
