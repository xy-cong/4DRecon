...
from models.sdfnet_base import SdfDecoder, SdfDecoder_Encoder
...
class SdfNet(nn.Module):
    def __init__(self,
        ...
        if config.model.sdf.use_xyz_encoder or config.model.sdf.use_time_encoder:
    ...
    def get_loss(self, time, batch_dict, state_info):
        ...
        if self.config.use_data_term:
            ...
            thresh = torch.ones_like(pred_sdf).to(device) * self.config.loss.truncated_dist
            
            # import ipdb; ipdb.set_trace()
            sdf_constraint = torch.where(gt_sdf != -1, torch.min(thresh, torch.max(pred_sdf, -thresh))-torch.min(thresh, torch.max(gt_sdf, -thresh)), torch.zeros_like(pred_sdf))
            state_info['surface_points_loss'] = torch.abs(sdf_constraint).mean().item()
            state_info['surface_points_weight'] = self.config.loss.data_term_surface_points_weight
            surface_loss = torch.abs(sdf_constraint).mean() * self.config.loss.data_term_surface_points_weight
            loss += surface_loss
        ...
        # # eikonal loss
        # if self.config.use_eikonal:
        #     nonmnfld_grad = gradient(batch_dict['sdf_pred'][:, :, None], batch_dict['query']) # (B, S, query_dim)
        #     eikonal_loss = ((nonmnfld_grad.norm(p=2, dim=-1) - 1) ** 2).mean() * self.config.loss.eikonal_weight
        #     loss += eikonal_loss
    ...
    def get_PD_loss(self, time, state_info):
        ...
        h1 = dgms[1]
        h2 = dgms[2]
        self.GT_pd_h0 = self.GT_pd_h0.to(time.device)
        self.GT_pd_h1 = self.GT_pd_h1.to(time.device)
        self.GT_pd_h2 = self.GT_pd_h2.to(time.device)
        if h0.shape[1] and self.GT_pd_h0.shape[1]:
            chamfer_distance_1 = chamfer_distance(repeat(h0,  '1 B d -> S B d', S=self.GT_pd_h0.shape[0]), self.GT_pd_h0, batch_reduction='mean')[0]
            sdf_topo_loss += chamfer_distance_1
        if h1.shape[1] and self.GT_pd_h1.shape[1]:
            chamfer_distance_2 = chamfer_distance(repeat(h1,  '1 B d -> S B d', S=self.GT_pd_h1.shape[0]), self.GT_pd_h1, batch_reduction='mean')[0]
            sdf_topo_loss += chamfer_distance_2
        if h2.shape[1] and self.GT_pd_h2.shape[1]:
            chamfer_distance_3 = chamfer_distance(repeat(h2,  '1 B d -> S B d', S=self.GT_pd_h2.shape[0]), self.GT_pd_h2, batch_reduction='mean')[0]
