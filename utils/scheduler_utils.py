...
def adjust_learning_rate(schedular_type, lr_scheduler, optimizer, epoch):
    if schedular_type == 'MultiplicativeLRSchedule':
        lr_group = lr_scheduler.get_lr_group(epoch)
        for i, param_group in enumerate(optimizer.param_groups):
            param_group["lr"] = lr_group[i]
        return lr_group
    elif schedular_type == 'OneCycleLR':
        lr_scheduler.step()
        lr_group = lr_scheduler.get_last_lr()
        return lr_group
    elif schedular_type == 'Step':
        for i, param_group in enumerate(optimizer.param_groups):
            param_group["lr"] = lr_scheduler[i].get_learning_rate(epoch) 
