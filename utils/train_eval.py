...
def train(model, epoch, optimizer, loader, lat_vecs, device,
    ...
    for b, data in enumerate(loader):
        optimizer.zero_grad()
        x = data.x.to(device) # (B, N=6890, 3)
        #x[:] = loader.dataset[4044].x
        ids = data.data_id.to(device) # (B, 1)
        #ids[:] = 4044
        batch_vecs = lat_vecs(ids.view(-1)) # (B, latent_dim)
        out = model(batch_vecs) # (B, N=6890, 3)
                
        pred_shape = out * data_std_gpu + data_mean_gpu # mean/std: (6890, 3)
